  ...
  function execQuery(db, query, options) {
    ...
    return pool.reserveAsync().then(function(connObj) {
      ...
      if(options.values) {
        ...
        _.forEach(options.values, function(val) {
          if(_.isInteger(val)) {
            promises.push(statement.setIntAsync(val));
          } else if(_.isString(val)) {
            promises.push(statement.setStringAsync(val));
          } else if(utils.isDate(val)) {
            promises.push(statement.setDateAsync(val));
          } else if(utils.isTimestamp(val)) {
            if(_.isDate(val)) {
              val = utils.toSqlDate(val);
            }
            promises.push(statement.setTimestampAsync(val));
          }
        });
