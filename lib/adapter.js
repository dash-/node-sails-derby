...
module.exports = (function () {
  ...
  var adapter = {
    ...
    query: function(connection, table, query, values, cb) {
      ...
      return execQuery(db, query, options).then(results => {
    ...
    queryUpdate: function(connection, table, query, values, cb) {
      ...
      execUpdate(db, query, options).then(results => {
        cb();
      }).catch(err => {
        cb(err);
      });
      ...
      Cursor({
        ...
        $populateBuffers: function populateBuffers(options, next) {
          ...
          execQuery(db, query).then(results => {
            ...
            _.forEach(parentRecords, parent => {
              var cache = {};

              _.forEach(_.keys(parent), key => {
            ...
            _.forEach(instructions.instructions, (population, popKey) => {
              ...
              _.forEach(parentRecords, parent => {
                ...
                if(! _.isUndefined(cachedChildren, alias)) {
                  _.forEach(cachedChildren[alias], cachedChild => {
            ...
            _.forEach(queries, query => {
              ...
              if(parentRecords.length > 1) {
                ...
                _.forEach(toSort, (sortVal, sortKey) => {
            ...
            function runQuery(preppedQuery) {
              ...
              return execQuery(db, query).then(results => {
                var groupedRecords = {};
                ...
                _.forEach(buffers.store, buffer => {
    ...
    return pool.reserveAsync().then(connObj => {
      ...
      if(options.values) {
        _.forEach(options.values, (val) => {
          if(_.isInteger(val)) {
            statement.setInt(val);
          } else if(_.isString(val)) {
            statement.setString(val);
          } else if(utils.isDate(val)) {
            statement.setDate(val);
          } else if(utils.isTimestamp(val)) {
            if(_.isDate(val)) {
              val = utils.toSqlDate(val);
            }
            statement.setTimestamp(val);
          }
        });
      }
